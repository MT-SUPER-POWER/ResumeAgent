...
- 在跨 Job 数据模型落地前使用模板 `Empty` 组件展示占位状态。
- 不把单次 Job 的候选人排名结果伪装为人才库数据。
...
- 使用 analytics 风格 KPI 条带展示开放岗位、覆盖部门、推荐次数和关联候选人。
- 使用模板 `Table`、`Badge`、`Select` 和 `Sheet` 构建岗位检索与详情工作流。
- 岗位详情必须明确展示其是否参与自动岗位匹配。
